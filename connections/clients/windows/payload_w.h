...
#ifndef WINDOWS_PAYLOAD_W_H_
#define WINDOWS_PAYLOAD_W_H_
...
struct PayloadDeleter {
  void operator()(connections::Payload* p);
};
}  // namespace connections
class InputFile;
class InputStream;
...
class DLL_API PayloadW {
 public:
  ...
  std::unique_ptr<connections::Payload, connections::PayloadDeleter> GetpImpl();

 private:
  std::unique_ptr<connections::Payload, connections::PayloadDeleter> pImpl;
};
