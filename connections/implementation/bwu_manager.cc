...
namespace {
const char kUnknownServiceId[] = "UNKNOWN_SERVICE";
}  // namespace
...
void BwuManager::InitiateBwuForEndpoint(ClientProxy* client,
  ...
  RunOnBwuManagerThread("bwu-init", [this, client, endpoint_id,
    ...
    SetMediumForEndpoint(endpoint_id, proposed_medium);
...
void BwuManager::OnEndpointDisconnect(ClientProxy* client,
  ...
  RunOnBwuManagerThread("bwu-on-endpoint-disconnect", [this, client, service_id,
                                                       endpoint_id,
                                                       barrier]() mutable {
    Medium medium = GetMediumForEndpoint(endpoint_id);
    ...
    // Note(nohle): I'm skeptical of the "<= 1". Luckily, we will enable the
    // flag by default, and it won't matter.
    if (FeatureFlags::GetInstance().GetFlags().support_multiple_bwu_mediums ||
        channel_manager_->GetConnectedEndpointsCount() <= 1) {
      RevertMediumForEndpoint(service_id, endpoint_id);
    }
    barrier.CountDown();
  });
}

void BwuManager::RevertMediumForEndpoint(const std::string& upgrade_service_id,
                                         const std::string& endpoint_id) {
  Medium medium = GetMediumForEndpoint(endpoint_id);
  ...
  endpoint_id_to_medium_.erase(endpoint_id);
  BwuHandler* handler = GetHandlerForMedium(medium);
  if (!handler) return;
...
Medium BwuManager::GetMediumForEndpoint(const std::string& endpoint_id) const {
  if (!FeatureFlags::GetInstance().GetFlags().support_multiple_bwu_mediums) {
    return medium_;
  }

  auto it = endpoint_id_to_medium_.find(endpoint_id);
  return it == endpoint_id_to_medium_.end() ? Medium::UNKNOWN_MEDIUM
                                            : it->second;
}
void BwuManager::SetMediumForEndpoint(const std::string& endpoint_id,
                                      Medium medium) {
  if (!FeatureFlags::GetInstance().GetFlags().support_multiple_bwu_mediums) {
    medium_ = medium;
    return;
  ...
  endpoint_id_to_medium_[endpoint_id] = medium;
}
...
void BwuManager::ProcessBwuPathAvailableEvent(
  ...
  Medium current_medium = GetMediumForEndpoint(endpoint_id);
  ...
  if (current_medium == Medium::UNKNOWN_MEDIUM) {
    SetMediumForEndpoint(endpoint_id, upgrade_medium);
  }
  // Check for the correct medium so we don't process an incorrect OfflineFrame.
  if (upgrade_medium != GetMediumForEndpoint(endpoint_id)) {
...
BwuManager::ProcessBwuPathAvailableEventInternal(
  ...
  Medium medium =
      parser::UpgradePathInfoMediumToMedium(upgrade_path_info.medium());
  if (medium != GetMediumForEndpoint(endpoint_id)) {
    NEARBY_LOGS(ERROR)
...
void BwuManager::RunUpgradeFailedProtocol(
  ...
  if (GetMediumForEndpoint(endpoint_id) != Medium::UNKNOWN_MEDIUM) {
    RevertMediumForEndpoint(channel->GetServiceId(), endpoint_id);
  }
  in_progress_upgrades_.erase(endpoint_id);
...
void BwuManager::ProcessSafeToClosePriorChannelEvent(
  ...
  client->GetAnalyticsRecorder().OnConnectionEstablished(
      endpoint_id, GetMediumForEndpoint(endpoint_id),
      client->GetConnectionToken(endpoint_id));
...
void BwuManager::ProcessUpgradeFailureEvent(
  ...
  std::string service_id =
      channel ? channel->GetServiceId() : kUnknownServiceId;
...
void BwuManager::RetryUpgradeMediums(ClientProxy* client,
  ...
  // retry upgrade.
  // TODO: Do we actually want to treat WIFI_LAN differently?
  auto channel = channel_manager_->GetChannelForEndpoint(endpoint_id);
  ...
  SetMediumForEndpoint(endpoint_id, next_medium);
