...
TEST(BleAdvertisementHeaderTest, Hash) {
  EXPECT_TRUE(absl::VerifyTypeImplementsAbslHashCorrectly({
      ...
      BleAdvertisementHeader(
          kVersion, true, 2,
          ByteArray(std::string("\x0c\x0d\x0e\x0f\x10\x11\x12\x13\x14\x15")),
          ByteArray(std::string("\x0E\x0F\x0G\x0H")), 1),
      BleAdvertisementHeader(
          kVersion, false, 4,
          ByteArray(std::string("\x20\x21\x22\x23\x24\x25\x26\x27\x28\x29")),
          ByteArray(std::string("\x0i\x0j\x0k\x0l")), 2),
  }));
}
