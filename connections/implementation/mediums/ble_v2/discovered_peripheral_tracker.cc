...
void DiscoveredPeripheralTracker::HandleAdvertisement(
  ...
  for (const std::string& uuid : advertisement_data.service_uuids) {
    // Caller UUID never equals kCopresenceServiceUuid. We de-dupe before
    // adding to AdvertiseData.
...
ByteArray DiscoveredPeripheralTracker::CreateDummyAdvertisementHeaderBytes(
  ...
  ByteArray advertisement_hash =
      BleUtils::GenerateAdvertisementHash(ByteArray(peripheral.GetId()));
